    ...
    DB::ErrorType: 'static,
{
    let mut chart = ChartBuilder::on(&b)
        .margin(1)
        .caption(title, ("noto-sans", (10).percent_height()))
...
pub fn draw(title: &str, submit_round: usize, data: Vec<f64>) -> Result<(), Box<dyn Error>> {
    let data_boundary = data
        .iter()
        .fold((f64::MAX, 0f64), |b, val| (b.0.min(*val), b.1.max(*val)));
    let data_seriers: Vec<(f64, f64)> = data
        .into_iter()
        .enumerate()
        .map(|v| ((v.0 + 1) as f64, v.1))
        .collect();
    draw_chart(
        TextDrawingBackend(vec![PixelState::Empty; 5000]).into_drawing_area(),
        title,
        0f64..(submit_round as f64) * 1.1,
        data_boundary.0 * 0.9..data_boundary.1 * 1.1,
        data_seriers,
    )?;
    Ok(())
}
